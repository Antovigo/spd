"""`jsp-pretrain <config.yaml>` — JAX next-token-CE pretraining of an in-house target LM.

The composition root and only I/O layer for pretraining; the step stays pure. Reuses the
decomposition trainer's substrate — `jax_single_pool.data` (offline pre-tokenized parquet,
never streamed), `jax_single_pool.sharding` (`init_distributed` / `dp_mesh`) — but the
...
from jax_single_pool.pretrain.cache import (
    cache_dir_for,
    torch_model_config_dict,
    write_pretrain_cache,
)
from jax_single_pool.pretrain.config import PretrainConfig, load_pretrain_config
from jax_single_pool.pretrain.models import PretrainModel, init_model, model_logits
...
def _cache_dir(cfg: PretrainConfig) -> Path:
    ...
    `jax_single_pool.llama_simple_mlp.pretrain_cache_dir` resolves. `out_dir` is the runs
