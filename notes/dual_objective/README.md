...
| per-position hidden normalization (S35 amended 2026-09-11) | `pd.hidden.normalization: {kind: per_position, floor_fraction: 0.01}` — `configs.PerPositionHiddenActsNormalization`, `losses.relative_squared_error`; default `batch` is the original, byte-inert |
...
| current SOTA recipe (reference, ungated) | `notes/dual_objective/addsub-L18-sota.yaml` — addsub-L18-23-neuronaligned (p-6540dfdd): the ntmerged-bsc recipe with `initialization: neuron_aligned_targeted` on the post-#1001 schema; outside the capped seat registry, so nothing migrates it. Full-model scale-up, one file set per pod shape: `addsub-all-layers-8xh100-sota.yaml` + `addsub-all-layers-8xh100-guide.md`, and `addsub-all-layers-4xh100-sota.yaml` + `addsub-all-layers-4xh100-guide.md`. Loss-balanced derivatives (2026-09-11, `addsub-all-layers/report.md`): `addsub-all-layers-4xh100-bal-coeff.yaml` (hidden coefficients x 32/14, no code change) and `addsub-all-layers-4xh100-bal-perpos.yaml` (the same plus `pd.hidden.normalization: per_position`) |
