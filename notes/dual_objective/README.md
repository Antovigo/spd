...
| current SOTA recipe (reference, ungated) | `notes/dual_objective/addsub-L18-sota.yaml` — addsub-L18-23-neuronaligned (p-6540dfdd): the ntmerged-bsc recipe with `initialization: neuron_aligned_targeted` on the post-#1001 schema; outside the capped seat registry, so nothing migrates it. Full-model scale-up, one file set per pod shape: `addsub-all-layers-8xh100-sota.yaml` + `addsub-all-layers-8xh100-guide.md`, and `addsub-all-layers-4xh100-sota.yaml` + `addsub-all-layers-4xh100-guide.md`. The 4xh100 sota started from the loss-balanced recipe of 2026-09-11 (`addsub-all-layers/report.md`, regenerated by `addsub-all-layers/make_final_run.py` from `addsub-all-layers-4xh100-bal-perpos.yaml`) and has since been carried through the `-02`..`-05` ladder (merged non-target adversary, doubled non-target minimality, the hidden pass's e2e KL rider, and finally the `neuron_aligned_zero_u` init — see the file's header for the full history). CANONICAL: `addsub-all-layers-4xh100-05` (`p-ba5a0c05`), confirmed as SOTA for the 32-block addsub task (Antoine, 2026-09-16). Earlier arms `-01`..`-04` (`p-ba1a0c01`..`p-ba4a0c04`) and the pre-balance run `p-b4132c01` are kept for comparison, in `~/out/pod-backup/`. The 8xh100 file set has NOT been rebalanced or carried through this ladder, and has never been run. UNTESTED VARIANT, authored 2026-09-17 and not part of the ladder: `addsub-all-layers-4xh100-06.yaml` — a separate copy of -05 with the CI-scaled weight decay narrowed to the target stream's hidden head (`stream: target`, `role: hidden`, `quantile: 1.0`) and `adv_fraction: 0.9` on all four merged recon terms at unchanged total coeffs. It runs under its own id `p-ba6a0c06`, which must be passed explicitly as `--run_id`: `PROFILES['4xh100']` still names the -05 sota and still carries `p-ba5a0c05`. |
| multiplication twin of the SOTA | `notes/dual_objective/mult-all-layers-4xh100-05.yaml` (`p-3c1a0c05`, authored 2026-09-21): `addsub-all-layers-4xh100-05` on the `a×b=` 1..100 pool (`prompts/multiplication_1-100.txt`), its own neuron ranks `mult1-100_llama31-8b_unit-energy`, and the ABGrid probe on `mul`; nothing else changes. Not in `PROFILES` — launch with `CONFIG=... RUN_ID=p-3c1a0c05 ./launch_run.sh --profile 4xh100`. |
| mult adversarial-share arm | `notes/dual_objective/mult-all-layers-4xh100-06.yaml` (`p-3c1a0c06`, authored 2026-09-22): mult-05 with `adv_fraction.max_val: 0.9` on all four merged recon terms (target + non-target, output + hidden) and the anneal stretched to the first 10000 steps (`at: 0.25`, vs -05's 2000); total `coeff` unchanged. Launch explicitly with `CONFIG=... RUN_ID=p-3c1a0c06`. |
| addsub wide-pool arm | `notes/dual_objective/addsub-all-layers-4xh100-08.yaml` (`p-ba8b0c08`, authored 2026-09-25; replaces the unlaunched 2026-09-20 softened-starts draft of the same name): the -05 SOTA with mult -06's adversarial schedule (`adv_fraction.max_val: 0.9` on all four merged recon terms, annealed over the first 10000 steps) and the pool widened to a, b in 1..400 (`prompts/addition_subtraction_1-400.txt`, 320000 prompts; neuron ranks `addsub1-400_llama31-8b_unit-energy`; ABGrid 400x400). Launch explicitly with `CONFIG=... RUN_ID=p-ba8b0c08`. |
