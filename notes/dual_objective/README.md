...
| current SOTA recipe (reference, ungated) | `notes/dual_objective/addsub-L18-sota.yaml` — addsub-L18-23-neuronaligned (p-6540dfdd): the ntmerged-bsc recipe with `initialization: neuron_aligned_targeted` on the post-#1001 schema; outside the capped seat registry, so nothing migrates it. Full-model scale-up, one file set per pod shape: `addsub-all-layers-8xh100-sota.yaml` + `addsub-all-layers-8xh100-guide.md`, and `addsub-all-layers-4xh100-sota.yaml` + `addsub-all-layers-4xh100-guide.md`. The 4xh100 sota IS the loss-balanced recipe as of 2026-09-11 (`addsub-all-layers/report.md`): regenerated by `addsub-all-layers/make_final_run.py` from `addsub-all-layers-4xh100-bal-perpos.yaml` (hidden coefficients x 32/14 plus `pd.hidden.normalization: per_position`) with the swept LR (1.6e-04/8.0e-05, half the originally authored value) and doubled C (124928 components). Running as `p-ba1a0c01` (`addsub-all-layers-4xh100-sota-01`); the pre-balance run `p-b4132c01` is in `~/out/pod-backup/`. The 8xh100 file set has NOT been rebalanced and has never been run. |
