...
1. Run one final `./pull_backup.sh --profile 4xh100 --once`, then confirm
   ...
   `ladder-4xh100/summary.md`; this last pass is what gets the FINAL checkpoint (53 GB; the
   `decomposition` item alone, ~10 GB, is what every consumer restores). Stop the backup loop
   afterwards.
