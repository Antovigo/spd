...
The run is referenced by id, so it resolves to `$DATA_ROOT/runs/p-ba5a0c05`."""
...
def main() -> None:
    ...
    print(
        f"{args.stage}: {args.out} (microbatch {config.microbatch_size}, eval {config.eval_batch_size})"
    )
