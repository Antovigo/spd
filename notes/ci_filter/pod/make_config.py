...
`CI_CEILING=1` sets `ci_ceiling: true` (CI can only decrease from the starting point's) on
every stage. The run is referenced by id, so it resolves to `$DATA_ROOT/runs/p-ba5a0c05`."""
...
def main() -> None:
    ...
    print(
        ...
        f"eval {config.eval_batch_size}, ci_ceiling {config.ci_ceiling})"
    )
