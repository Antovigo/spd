...
with `a >= b` (5,050). Everything decomposition-specific lives in the run dir, `<run_dir>/analysis/arith_repr/` (`/mnt/nw/home/a.vigouroux/out/pod-backup/p-ba5a0c05/analysis/arith_repr`: `V_alive.npz`, `analysis/`, `figures/`, `app/`, `synth_app/`). The residual stream of the frozen model (raw, every block boundary; a property of the model, not of the decomposition) lives in `/mnt/nw/home/a.vigouroux/out/pod-backup/original-resid/addsub1-100_llama31-8b/` (README there); the analysis recovers each read point's post-norm input from it with the model's RMSNorm weights (`param_decomp/arith_repr/resid.py`); code
