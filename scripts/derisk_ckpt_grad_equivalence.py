...
def run_variant(
    ...
    *,
    ...
    with autocast:
        for i, block in enumerate(model):
            ...
            match delivery:
                case "arg":
                    h = (
                        checkpoint(block, h, mask, use_reentrant=False)
                        if use_ckpt
                        else block(h, mask)
                    )
                case "arg_dict":
    ...
    for i, block in enumerate(model):
        ...
        grads[f"b{i}.mlp"] = (
            None if block.mlp.weight.grad is None else block.mlp.weight.grad.clone()
        )
...
def max_grad_diff(
    ref: dict[str, Tensor | None], got: dict[str, Tensor | None]
) -> tuple[float, str]:
    ...
    for k, rg in ref.items():
        ...
        if rg is None or gg is None:
            if (rg is None) != (gg is None):
                return float(
                    "inf"
                ), f"{k}: ref={'None' if rg is None else 'T'} got={'None' if gg is None else 'T'}"
            continue
    ...
    for bf16 in (False, True):
        ...
        for delivery, use_ckpt, expect in variants:
            try:
                ...
                print(
                    f"{delivery + ' ckpt':>26} | {'RAISED':>10} | {'broke':>14} | {msg[:30]}  [{expect}]"
                )
                continue
