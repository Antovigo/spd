...
run p-ecfda851's run_meta.yaml). Changes:
  ...
  - target.activation_checkpointing -> true, but LW-pool-only (gated in optimize.py): LW
    carries the full per-rank batch (bl_lw=256) so it needs ckpt; CI/PPGD run plain at bl=8.
  - topology -> 96 LW blocks (1 site/block, 1 GPU/block, bl_lw=256) + CI 32 + PPGD 32 = 160 GPU
    (bl_ci=bl_pp=8; bl=16 OOMs CI+PPGD and ckpt doesn't help them — lore batch-256-push-decision).
  - synchronized LR warmup: warmup_pct = 0.01 (500/50000 steps) on comp, ci_fn AND ppgd.
  - 2-run LR sweep (keeping the 5:1 comp:ci ratio): A comp 5e-4 / ci 1e-4 (= the proven b16
    baseline, un-scaled), B comp 1e-3 / ci 2e-4. This is 10x below the first sweep
    (5e-3 / 1e-2), which was unstable (CI-fn thrash, L0 collapse) — sqrt-projection from b48
    was far too hot, so b256 wants ~no LR scaling.
...
Also emits a smoke variant (steps 30, save_every 20, faith warmup 2) at the SAME 160-GPU
...
def build_topology() -> dict[str, Any]:
    return {
        ...
        "layerwise_block_groups": [{"ranks": [i], "owned_sites": [SITES[i]]} for i in range(N_LW)],
    }


def make_cfg(base: dict[str, Any], comp_lr: float, ci_lr: float) -> dict[str, Any]:
