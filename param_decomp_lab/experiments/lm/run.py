"""The LM decomposition composition root: wrapper YAML -> a full decomposition run on a
vendored target.
...
def _make_lm_eval_fn(
    ...
    def eval_fn(state: TrainState, now_step: int) -> "LogRecord":
        ...
        # docstring for the per-key parity argument.
        ...
        if eval_batches and slow_due:
            # SLOW/PLOT TIER. The COLLECTIVE part runs in lockstep on every
