...
class TargetConfig:
    ...
    convert time — no silent downgrade."""
...
# run in the IN-LOOP SLOW TIER (in-loop only — no offline CLI), on cadence
...
def assert_supported_weights_dtype(cfg: LMExperimentConfig) -> None:
    """Refuse a frozen-target weights_dtype the loader can't honour (no
    ...
    assert cfg.target.weights_dtype in target.supported_weights_dtypes, (
        ...
        f"{cfg.target.weights_dtype!r}. No silent downgrade: declare a "
