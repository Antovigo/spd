...
the dataset at launch makes every rank hit HF Hub for parquet shards; at high rank counts
that thundering herd read-times-out / RemoteDisconnects, stranding a rank before the
`build_two_world` collective and hanging startup. Pre-tokenizing to local Arrow/parquet
removes the runtime HF dependency AND the per-rank tokenization cost.
