"""Scatter the activations projected onto each accepted ridge-CV probe plane.

For every (variable, period) cell of a `ridge_cv_probes_<op>.json`, picks the accepted
layer with the highest `cv_r2` and scatters a subsample of per-prompt activations at
`(x·w_cos + b_cos, x·w_sin + b_sin)` — the probe's predicted (cos, sin) plane — colored
by `v mod T` on a cyclic colormap, so a genuine circular representation shows as an
ordered color wheel. Period 2 (sin ≡ 0) is drawn as a 1-D strip with jitter. Cells with
no accepted layer render empty ("gated").

Usage:
    python -m param_decomp_lab.scripts.validation.probes.plot_probe_projections \
        <ridge_cv_probes_<op>.json> [--resid-stream-npz=PATH] [--alpha=0.05] \
        [--n-show=3000] [--seed=0] [--output=PATH] [--slurm [--gpus=0 ...]]

Output (default beside the json): `probe_projections_<op>.png`. The npz defaults to the
json's `meta.source` (the `resid_stream_<op>.npz` it was fit on).
"""
...
def plot_probe_projections(
    ...
    def best_layer(variable: str, period: int) -> str | None:
        cells = {
            ...
            if cell[variable][str(period)]["p_value"] <= alpha
            and cell[variable][str(period)]["cv_r2"] > 0
        }
    ...
    proj: dict[tuple[str, int], NDArray[np.float32]] = {}
    for layer_key in sorted(set(chosen.values())):
        ...
        for (variable, period), lk in chosen.items():
            ...
            if cell["w_sin"] is None:
                proj[(variable, period)] = cos_pred[:, None]
            else:
                w_sin = np.asarray(cell["w_sin"], dtype=np.float32)
                proj[(variable, period)] = np.stack([cos_pred, x @ w_sin + cell["b_sin"]], axis=1)
        del x
    ...
    for row_i, variable in enumerate(variables):
        values = _variable_values(aa, bb, variable)[show]
        for col_i, period in enumerate(periods):
            ...
            if (variable, period) not in proj:
                ax.set_facecolor("#f0f0f2")
                ax.text(
                    0.5,
                    0.5,
                    "gated",
            ...
            if p.shape[1] == 1:  # period-2: 1-D probe, jitter the empty axis
                ...
                circle = plt.Circle((0, 0), 1.0, fill=False, color="#999999", lw=0.6, ls=":")
                ax.add_patch(circle)
                ax.set_aspect("equal")
            ax.text(
                0.02,
    ...
    plt.close(fig)
    logger.info(f"wrote {out_path} ({len(proj)}/{len(variables) * len(periods)} panels drawn)")
    return out_path
