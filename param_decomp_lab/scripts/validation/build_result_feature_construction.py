...
probes fit to the residual stream **after** the MLP (`probes_post.json`, site `post`), in the
probes' predicted `(cos, sin)` frame — three rows: the residual **before** the MLP, **after**
the MLP, and after the MLP **with a user-picked set of neurons or subcomponents ablated**.
...
Usage:
    python -m param_decomp_lab.scripts.validation.build_result_feature_construction \
        <model_path> [--probes=.../probes_post.json] [--census-dir=PATH] [--kl-thr=0.01] \
        [--periods=2,5,10,20,50,100] [--rank=64] [--stride=2] [--output-dir=PATH]

Reads: the probes JSON (default `<PARAM_DECOMP_OUT_DIR>/runs/fourier_probes/probes_post.json`),
the run's `hidden_activations_add.npz` / `alive_filtered_add.tsv` / `subcomp_periods_add.tsv`,
the census `candidates.tsv` (+ `ablation_full_add.npz` when present, overriding the screen KL),
and the checkpoint U/V + frozen MLP weights (mmap).
...
def _probe_axes(
    probes: dict[str, Any], periods: list[int], x_post: NDArray[np.float32]
) -> tuple[NDArray[np.float32], NDArray[np.float32], list[float]]:
    """Per period, the predicted-`(cos, sin)` frame: `W [d_model, 2P]`, `bias [2P]`, mean r².

    A degenerate sin probe (period 2, `sin ≡ 0`) falls back to the top post-cloud variance
    direction ⊥ the cos axis, rescaled to the cos-prediction spread and mean-centred, so the
    residue split stays a 2D plot (as in `build_fourier_scatter`).
    """
    d_model = x_post.shape[1]
    ...
    for j, t in enumerate(periods):
        ...
        if w_sin is not None and float(np.linalg.norm(w_sin)) > 1e-6:
            ...
            xc = x_post - x_post.mean(axis=0)
            xc = xc - np.outer(xc @ e1, e1)
            e2 = np.linalg.eigh(xc.T @ xc)[1][:, -1].astype(np.float32)
            e2 /= max(float(np.linalg.norm(e2)), 1e-12)
            scale = float(np.std(x_post @ w_cos + b[2 * j])) / max(
                float(np.std(x_post @ e2)), 1e-12
            )
            w[:, 2 * j + 1] = e2 * scale
            b[2 * j + 1] = -float((x_post @ e2).mean()) * scale
...
def _exact_gu_delta(
    ...
    wd: NDArray[np.float32],
) -> NDArray[np.float32]:
    """Exact projected-output delta `[N, 2P]` for ablating gate/up components together.

    `sel` rows are `(proj, inner [N], U_row [d_int])`; the SwiGLU is re-evaluated with each
    component's rank-1 term removed from its preactivation.
    """
    dg = np.zeros_like(gate)
    du = np.zeros_like(up)
    for proj, inner, u_row in sel:
        (dg if proj == "gate_proj" else du)[...] += np.outer(inner, u_row)
    return ((silu_combine(gate - dg, up - du) - act) @ wd).astype(np.float32)
...
def build_result_feature_construction(
    ...
    probes_path = (
        Path(probes).expanduser()
        if probes
        else PARAM_DECOMP_OUT_DIR / "runs" / "fourier_probes" / "probes_post.json"
    )
    assert probes_path.exists(), f"missing probes JSON: {probes_path}"
    probes_payload = json.loads(probes_path.read_text())
    assert probes_payload["site"] == "post", (
        f"probes must be fit at the post-MLP residual site, got {probes_payload['site']!r}"
    )
    ...
    layer = int(d["layer"])
    assert layer == int(probes_payload["layer"]), (
        f"probe layer {probes_payload['layer']} != run layer {layer}"
    )
    n = int(d["a"].shape[0])
    n_prompts = n * n
    ...
    w_ax, b_ax, r2s = _probe_axes(probes_payload["probes"]["a+b"], period_list, resid_post)
    proj_pre = resid_pre @ w_ax + b_ax
    proj_post = resid_post @ w_ax + b_ax
    logger.info(
        "probe planes: "
    ...
    for proj, c in gu_comps:
        exact_single[(proj, c)] = _exact_gu_delta(
            gate, up, act, [(proj, inner_grids[(proj, c)], uv[proj][1][c])], wd
        )
    ...
    for label, sel in checks:
        ...
        dex = _exact_gu_delta(gate_s, up_s, act_s, rows, wd)
        dem = _exact_gu_delta(recon["g"], recon["u"], act_r, rows, wd)
    ...
    payload = {
        "meta": {
            ...
            "periods": period_list,
            "r2": r2s,
            "probes": probes_path.name,
            "mods": sorted({p for p in period_list if p > 1}),
            "fidelity": fidelity,
        },
        "proj_pre": b64_f16(proj_pre),
        "proj_post": b64_f16(proj_post),
