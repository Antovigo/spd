...
A class is *present* at share ≥ `--theta`; `n_periods` = number of present classes:
0 = aperiodic (fine), 1 = clean single period (fine — a both-operand blob grid is one
period), ≥ 2 = mixing, the failure mode being measured. `mixed_frac` / `excess_periods`
aggregate over periodic components only. The θ cut is sharp on real runs, so the summary
reports mixed_frac at θ ∈ {0.1, 0.2, 0.3} plus the θ-free `secondary_share` (mean share of
the second-strongest class).

Usage:
    python -m param_decomp_lab.scripts.validation.score_period_separation <model_path> \
        [--ci-gate=0.1] [--theta=0.2] [--module-grep=mlp] [--batch-size=512] \
...
per-class shares, present periods, n_periods, secondary_share),
`analysis/datasets/period_separation_summary.tsv` (per matrix + pooled: n_active,
periodic_frac, mixed_frac at the three θs, excess_periods, secondary_share, census), and
...
def _summary_row(scored: "list[ComponentPeriods]", theta: float) -> dict[str, object]:
    n_periods = [count_periods(s.shares, theta) for s in scored]
    periodic = [n for n in n_periods if n >= 1]
    row: dict[str, object] = {
        "n_active": len(scored),
        "periodic_frac": round(len(periodic) / len(scored), 4) if scored else "",
    }
    for side_theta in _THETAS:
        nps = [count_periods(s.shares, side_theta) for s in scored]
        per = [n for n in nps if n >= 1]
        row[f"mixed_frac_t{int(side_theta * 100):02d}"] = (
    ...
    census: dict[int, int] = {}
    for s in scored:
        for period, share in s.shares.items():
            if share >= theta:
...
def score_period_separation(
    ...
    if slurm:
        ...
        argv += [f"--ci-gate={ci_gate}", f"--theta={theta}", f"--module-grep={module_grep}"]
    ...
    fields += [f"share_T{p}" for p in periods] + ["secondary_share"]
    with open(out_path, "w", newline="") as f:
        ...
        for s in sorted(scored, key=lambda s: (parse_module_name(s.module), s.component)):
            ...
            present = sorted((p for p in periods if s.shares[p] >= theta), reverse=True)
            row: dict[str, object] = {
    ...
    summary_fields += [f"mixed_frac_t{int(t * 100):02d}" for t in _THETAS]
    ...
    with open(out_summary_path, "w", newline="") as f:
        ...
        for layer, matrix in matrices:
            ...
            writer.writerow({"layer": layer, "matrix": matrix, **_summary_row(group, theta)})
        writer.writerow({"layer": "", "matrix": "all", **_summary_row(scored, theta)})

    fig = metric.plot_panel(scored)
    fig.savefig(out_fig_path, dpi=250)

    pooled = _summary_row(scored, theta)
    logger.info(
        f"{len(scored)} subcomponents scored (CI > {ci_gate}) — "
        f"mixed_frac@{theta}={pooled[f'mixed_frac_t{int(theta * 100):02d}']}, "
