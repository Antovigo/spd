...
def _frozen_model() -> GPT2Simple:
    ...
    m = GPT2Simple(
        cfg
    )  # weights init from the model's own seed-42 generator, independent of global RNG
...
def _build_pair() -> tuple[ComponentModel, LMComponentModel]:
    ...
    cm = ComponentModel(_frozen_model(), _run_batch, targets, ci_config, sigmoid_type="leaky_hard")
    torch.manual_seed(SEED)
