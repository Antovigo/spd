"""Every config yaml the repo maintains must parse at tip — the LM seats against
`LMExperimentConfig`, the toy seats against their domain schemas.

A schema change that breaks a config here migrates it in the same change, with an
executed in-repo migration — never a script attached to a review comment (a migration
that lives outside the repo never runs).
"""
...
# Archetype seats predating the modern schema, awaiting migration. When you
# fix one, remove it here — the gate then covers it; this list must only ever
# shrink.
KNOWN_BROKEN = {
...
def test_gate_collects_the_seat_registry() -> None:
    ...
    seat policy: non-empty, and within the 10-LM-config registry cap."""
    assert CONFIG_PATHS, "config glob collected nothing — did the config roots move?"
    assert len(CONFIG_PATHS) <= 10, (
        f"{len(CONFIG_PATHS)} LM configs exceed the registry cap of 10 — "
        "adding a seat requires an eviction (or this is an uncommitted one-off)"
    )
...
def test_config_parses_and_is_canonical(path: Path) -> None:
    ...
    if rel in KNOWN_BROKEN:
        ...
        pytest.skip(f"{rel}: known-broken seat awaiting migration")
