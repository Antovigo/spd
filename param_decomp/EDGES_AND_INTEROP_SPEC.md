...
| E28 | Supported PD subspace is enforced two ways. Leaky-hard sigmoid, the always-built delta component, no tied weights, and no identity decomposition targets are enforced by REMOVAL — those fields are gone from `PDConfig`, so `extra=forbid` rejects any config that sets them (no assert needed). The rest is asserted, not approximated: persistent scope `sc` only, AdamW `wd=0`/betas `(0.9,0.999)`, cosine-to-`0.1×` no-warmup LR, components grad-clip set / ci-fn unclipped, faith-warmup `wd==0` (`assert_canonical_algorithm_config`). | `configs.py` (`PDConfig`, `extra=forbid`); `experiments/config.py` (`assert_canonical_algorithm_config`); SPEC `S19`, `S20`, `S21` |
