...
def init_persistent_sources(
    ...
    """Per-site PPGD sources `(*leading_shape, C+1)`, init U[0,1] (clamp
    parameterization); trailing channel = the weight-delta source. `leading_shape` spells
    the `source_shape` over the model's leading axes, rank matching the
...
def sources_adam_ascend_project(
    ...
    The variation point `SRC_STEP`: a `sign` variant would replace the Adam
...
def source_masks(
    ...
    """`mask = ci + (1−ci)·source[:, :C]`; delta mask = raw trailing channel.
...
def per_sample_adversarial_assignment(
    ...
    position axes — a whole sample takes one mask family."""
...
def mixed_persistent_stochastic_masks(
    ...
    """Forward inputs for the merged stochastic+PPGD term. Adversarial-assigned
    ...
    final ascent rides this term's backward."""
...
class PersistentAdversary(eqx.Module):
    """One persistent-PGD adversary: the per-site sources + their Adam moments
    ...
    term's `coeff` — exact since one source bundle feeds exactly one term)."""
    ...
    def final_ascend(
        ...
        """One final ascent from the shared backward's source-grad. The
        backward saw `coeff·L_term`, so the grad is unscaled by `coeff` to ascend on
        `L_term` itself (exact: each source bundle feeds exactly one term)."""
        lr = self.source_lr(step_f32, total_steps)
