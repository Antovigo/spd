...
"""Which reconstruction objective a CI value scores for (SPEC S37). `output` is the
...
@dataclass(frozen=True)
class DualCI:
    """Two `CI` bundles read off ONE trunk (SPEC S37): `output` feeds the output-reconstruction
...
class PlacedCIFn(eqx.Module):
    ...
    @property
    def capture_keys(self) -> CaptureKeys:
        """The wrapped fn's taps — the pair answers the same question the fn does."""
        return self.fn.capture_keys

    @property
    def roles(self) -> tuple[CIRole, ...]:
        return self.fn.roles
...
def evaluate_ci(placed_ci_fn: PlacedCIFn, taps: dict[str, Array], *, remat: bool) -> AnyCI:
...
def evaluate_compute_ci(compute_ci_fn: PlacedCIFn, taps: dict[str, Array], *, remat: bool) -> AnyCI:
...
def evaluate_ci_role(
    placed_ci_fn: PlacedCIFn, taps: dict[str, Array], *, remat: bool, role: CIRole
) -> CI:
    """One role's bundle — the seam for consumers (evals, probes) that score a single role."""
    return ci_for_role(evaluate_ci(placed_ci_fn, taps, remat=remat), role)
...
def ci_preactivations(
    ...
    numbers under both names — the exact failure S37 names."""
    compute_ci_fn = materialize_ci_compute_weights(placed_ci_fn)
    ci = ci_for_role(evaluate_compute_ci(compute_ci_fn, taps, remat=remat), role)
...
class ChunkTransformer(eqx.Module):
    ...
    """The SECOND readout head (SPEC S37), empty tuples in a single-role fn. `in_proj_*` and
    ...
    def shardings(self, mesh: Mesh, placement: CIFnPlacement) -> "ChunkTransformer":
        ...
        return eqx.tree_at(
            ...
            (
                ...
                tuple(
                    _vector_sharding(vectors, ("stack", "C"), bias.shape)
                    for bias in self.hidden_out_bs
                ),
            ),
        )
    ...
    def __call__(
        ...
        *,
        ...
        for block in self.blocks:
            x = block(x, inv_freq, placement=placement, valid_token_count=valid_token_count)

        def readout(
            ws: tuple[Array, ...], bs: tuple[Array, ...]
        ) -> tuple[Float[Array, "*leading _C"], ...]:
            return tuple(
                _ci_linear(x, w, placement, "output", CI_OUTPUT_AXES[1:], transposed=False) + b
...
def _reconstruct_ci_compute_weights(
    ...
    return eqx.tree_at(
        ...
        (
            ...
            tuple(pin(w, output, CI_OUTPUT_AXES) for w in chunks.hidden_out_ws),
        ),
    )
...
class ChunkwiseTransformerCIFn(eqx.Module):
    ...
    def __call__(
        ...
        *,
        ...
        return self._evaluate(taps, remat=remat, placement=placement, valid_token_count=None)
    ...
    def evaluate_padded(
        ...
        *,
        ...
        """Evaluate without letting right-padding into the chunks' attention (SPEC §4.6)."""
        return self._evaluate(
            taps, remat=remat, placement=placement, valid_token_count=valid_token_count
        )
...
class SiteMLP(eqx.Module):
    ...
    """The SECOND readout head (SPEC S37) — a replacement for the FINAL layer only, `None` in a
    ...
    def role_preactivations(
        ...
        def linear(x: Array, w: Array, b: Array) -> Array:
            ...
            return jnp.einsum("...i,io->...o", x, w, out_sharding=P(*leading_spec, None)) + b
...
def build_ci_fn(arch: CIFnArch, sites: tuple[SiteSpec, ...], key: PRNGKeyArray) -> CIFn:
    ...
    `arch.dual` builds the second readout head off the SAME trunk (SPEC S37) — every impl
