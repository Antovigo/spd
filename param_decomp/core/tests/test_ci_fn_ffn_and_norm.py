...
def test_learned_norm_scale_inits_to_ones_so_step_zero_is_unchanged():
    ...
    assert jnp.allclose(
        output_ci(scaled(_taps(), remat=False, placement=None)).preactivations[SITES[0].name],
        output_ci(weightless(_taps(), remat=False, placement=None)).preactivations[SITES[0].name],
        rtol=1e-6,
        atol=1e-6,
    )
...
def test_swiglu_is_the_gated_product_not_a_gelu():
    ...
    base = output_ci(swiglu(_taps(), remat=False, placement=None)).preactivations[SITES[0].name]
...
def test_swiglu_and_gelu_differ():
    a = output_ci(_build("gelu", False)(_taps(), remat=False, placement=None)).preactivations[
        SITES[0].name
    ]
    b = output_ci(_build("swiglu", False)(_taps(), remat=False, placement=None)).preactivations[
        SITES[0].name
    ]
...
def test_every_combination_runs_end_to_end(ffn_kind: str, learned_norm_scale: bool):
    ci = output_ci(_build(ffn_kind, learned_norm_scale)(_taps(), remat=True, placement=None))
    for site in SITES:
