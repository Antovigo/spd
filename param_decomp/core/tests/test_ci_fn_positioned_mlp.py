...
def test_mlp_arch_serves_a_positioned_target():
    ...
    ci = output_ci(ci_fn(_taps(), remat=False, placement=None))
...
def test_evaluate_ci_casts_fp32_taps_to_compute_precision():
    ...
    assert (
        output_ci(ci_fn(taps, remat=False, placement=None)).preactivations[SITE].dtype
        == jnp.float32
    )
    assert (
        evaluate_ci_role(PlacedCIFn(fn=ci_fn, placement=None), taps, remat=False, role="output")
        .preactivations[SITE]
        .dtype
        == COMPUTE_DT
    )
...
def test_positioned_mlp_is_position_local():
    ...
    moved = jnp.abs(
        output_ci(ci_fn(taps, remat=False, placement=None)).preactivations[SITE]
        - output_ci(ci_fn(perturbed, remat=False, placement=None)).preactivations[SITE]
    ).max(axis=(0, 2))
...
def test_positioned_mlp_reads_tap_magnitude_where_the_blockless_chunk_cannot():
    ...
    def scale_gap(ci_fn: CIFn) -> float:
        x = _taps(1)[SITE]
        f = lambda t: output_ci(ci_fn({SITE: t}, remat=False, placement=None)).preactivations[SITE]  # noqa: E731
