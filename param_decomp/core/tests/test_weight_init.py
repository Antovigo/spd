...
def _hand_alignment(n_neurons: int) -> NeuronAlignment:
    """A fixed ranking `S` shared by the block's three MLP sites; each takes its prefix."""
    from param_decomp.targets.glu_transformer import site_name

    ranking = np.array([5, 0, 9, 2, 7, 1, 8, 3], dtype=np.int32)
    assert ranking.max() < n_neurons
    return {
...
def test_neuron_aligned_init_reads_back_the_selected_neurons_and_leaves_the_rest_to_the_delta():
    ...
    for name, site_alignment in alignment.items():
        ...
        site = vu.site(name)
        composed = (site.V @ site.U).T
        expected = jnp.zeros_like(W)
        if site_alignment.neuron_axis == "d_out":
            expected = expected.at[S, :].set(W[S, :])
            # x @ V is the selected neurons' pre-activation; U is one-hot rows.
            assert jnp.array_equal(site.V, W[S, :].T)
            assert jnp.array_equal(site.U, jax.nn.one_hot(S, spec.d_out))
        else:
            expected = expected.at[:, S].set(W[:, S])
            assert jnp.array_equal(site.V, jax.nn.one_hot(S, spec.d_in).T)
            assert jnp.array_equal(site.U, W[:, S].T)
        assert jnp.allclose(composed, expected, atol=1e-6), name
        assert jnp.allclose(site_weight_delta(deltas, vu, name), W - expected, atol=1e-5), name
        # Each subcomponent carries exactly its neuron's weight norm.
        norms = jnp.linalg.norm(site.V, axis=0) * jnp.linalg.norm(site.U, axis=1)
        axis = 1 if site_alignment.neuron_axis == "d_out" else 0
        selected = W[S, :] if axis == 1 else W[:, S].T
        assert jnp.allclose(norms, jnp.linalg.norm(selected, axis=1), atol=1e-5), name
...
def test_neuron_alignment_validation_refuses_bad_indices():
    ...
    good = _hand_alignment(tiny_glu_cfg().n_intermediate)
