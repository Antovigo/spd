...
def masked_forward(batch, live_sites, masks, delta_masks, routes, wanted) -> ForwardResult:
...
| S3 | The recon target is the `.output` of the step's all-frozen `clean_forward(batch, clean_capture)`: the frozen-path full model (embed the token batch, all blocks, final norm, target head), stop-gradient. Never the `mask=1, delta=1` decomposed identity (differs in bf16 and pollutes the graph). A subset decomposition simply leaves non-decomposed blocks on the frozen `x @ W` path. **AMENDED 2026-06-24** (Oli-approved): removed residual-start — the model takes the target batch and embeds internally, so the clean forward is the whole-model frozen forward (was: a suffix over a separately harvested residual). See REMOVE_RESIDUAL_START_DESIGN.md. **AMENDED 2026-07-30** (Dan/Oli design review): the specialized `clean_output` protocol method is retired into the empty-key arm of the unified clean-forward result; semantics are unchanged. **PERFORMANCE AMENDMENT 2026-08-13, PENDING OLI**: a target may declare `split_layer > 0` and accept a `ResidualStart` (`model.ResidualStart`) in place of its token input — the frozen lead below the first decomposed block, computed once per stream per step and shared by every forward. This does NOT reinstate the removed residual-start: the model still owns the whole forward and embeds tokens internally, the reused value is produced by the model's own `prefix_residual` inside the same step (never harvested, never stored, never crossing a run boundary), and the recon target is unchanged. Masking cannot reach blocks below `split_layer`, so every forward in a step computes an identical lead; sharing it is numerically identical (verified bit-exact in `targets/tests/test_prefix_reuse.py`), not merely within tolerance. |
...
| S14′ | Each persistent term's final ascent gradient comes from the SAME graph as the main backward (pre-update components, live `ci_lower`) and IS `dL_term/d(sources)` directly: a persistent-carrying term's coeff is applied to its MODEL-SIDE cotangents only (`model_cotangents_scaled` wraps the term's prepared weights + CI envelope — bit-identical forward, cotangents scaled by the per-step coeff) and the term enters the differentiated total at weight 1, so the source path is never coeff-scaled and NOTHING is unscaled. Consequences: components/CI receive exactly `coeff·dL/dθ` (an exact 0 while an activation-gate schedule holds the coeff at 0, no division anywhere), and the final ascent runs full-strength at EVERY step — through gates included — alongside the coeff-free S24 warmup ascents. The reported `total` metric stays the objective `Σ coeff·L` (identical values: the wrapper is forward-exact). Applied after backward; must not use post-update params. **Amended 2026-08-06 twice** (schedulable coefficients; then coeff moved to model-side cotangents) — pending Oli's normative sign-off. |
...
| S35 | Per-term hidden-acts reconstruction rider (#94): any recon loss config may carry `hidden_acts_reconstruction: {coeff, points}`, adding `coeff · mean_points(relative squared error at the named activations)` to THAT term's masked-vs-clean comparison on every draw — sharing the term's forwards (the points ride the draw's capture keys; no extra forward). `points` name activations in the TARGET's own tap vocabulary and have NO default (which internal activations matter is an experiment question). Per-point normalization is by the clean activation's own squared scale. Logged per point under `loss/<term>/hidden_acts_reconstruction/<point>` plus the term aggregate. Distinct from S31's standalone eval metrics, which remain eval-only. The rider is TARGET-PASS-ONLY vocabulary: a tPD non-target term refuses it (T5). **Added 2026-08-05** — this row documents shipped #94 behavior whose code already cited S35. |
| S36 | Dual (shared-trunk) causal importance. A CI fn may declare TWO readout ROLES over ONE trunk (`decomposition.ci.dual`): `output` scores subcomponents for reconstructing the model OUTPUT, `hidden` for reconstructing named internal activations (T12). Everything before the final readout — the input projector and every block, or the GELU stack in the MLP arches — is ONE set of arrays in the parameter tree, so the trunk is forward-run once, backward-run once, and its gradient is the SUM of the two roles' cotangents through a single `eqx.filter_vjp` pullback. Only the `d_model -> C_j` head is private per role, because the head is where "how important is this subcomponent FOR THIS OBJECTIVE" lives; everything before it is "read the model's state", which is role-independent by hypothesis — and testing that hypothesis is what a dual run is for. Both roles score the SAME subcomponent pool over the SAME inputs. A single-role fn returns a bare `CI` and never mentions the vocabulary; a dual fn returns `DualCI`, and every consumer NAMES the role it reads (`ci_for_role`) rather than reaching through the union — a silent default would be exactly the class of bug that makes a dual run report one head's numbers under both names. The hidden head's init folds off the output head's own key rather than widening the RNG split, so a single-role fn is bit-identical to the pre-S36 one AND a dual run's trunk and output head match a single-role run at the same seed: the two topologies are comparable from step 0. Dual is TARGETED-ONLY — a plain objective refuses a dual CI fn at trace, since its second head would have no loss to answer to. A dual CI fn and a hidden pass must be declared together (refused at config build). Distinct from the torch `GlobalSharedTransformerCiFn.adopt_trunk`, which shares by MODULE IDENTITY and therefore needs both a hand-rolled parameter dedupe and a load-time value comparison to tell the two topologies apart; here the trunk appears once in the pytree and the pytree SHAPE differs, so neither trap is reachable. **Added 2026-08-18.** |
...
coefficiented `ReconPlan` / `ReconForward` terms, and
...
The NON-TARGET pass is delta-pinned reconstruction against the broad stream's frozen
output, the unmasked-no-delta term (T4's one delta-off exception), importance-minimality
at its own coefficient — and, since the 2026-08-19 T5/T7 amendment, optionally
COMPONENT-ONLY persistent adversarial reconstruction (the delta stays pinned; the
bundle's delta channel is ignored). The original refusal argument — with the delta
pinned on, adversarial probing and internal-activation matching would constrain the
very behavior tPD declines to decompose — binds the DELTA path and internal-activation
matching, both of which remain excluded. The non-target surface is **authored
directly** (`configs.NontargetConfig`), never derived from the target pass's loss list
— there is no keep/drop filtering anywhere. The non-target HIDDEN pass's admitted
types are the closed set (`NontargetReconLossMetricConfig`: the stochastic/constant-source
recon types plus `UnmaskedNoDeltaReconLoss`) plus, since the 2026-08-20 amendment, the
MERGED persistent type only (`NontargetHiddenReconLossMetricConfig`) — the one
adversarial arm that rides an existing masked forward; the OUTPUT pass's union adds both
persistent types (`NontargetOutputReconLossMetricConfig`).
...
| id | invariant |
|---|---|
| T1 | One optimizer step = `∇(Σ_passes L_pass)` over the run's passes (two without a hidden objective, up to four with one — T12); every pass sees the same pre-update parameters, the same live CI fn, and the same annealed schedule parameters. The CI fn's gradient is the sum of the two STREAMS' pullbacks (each of which already carries every head's cotangent, S36). **AMENDED 2026-08-18**: was "from a single `value_and_grad`". The passes may be scored FUSED (one `value_and_grad` over their sum) or SEQUENTIALLY (one per pass, gradients added, with `lax.optimization_barrier` threading the trainables through the accumulator so XLA cannot hoist the next pass's forwards ahead of the previous pass's backward) — `runtime.sequential_passes`. The two are the SAME objective: summing per-pass gradients is precisely what the fused backward does internally, and each pass differentiates the same trainable tuple (a pass that does not read a leaf contributes a zero cotangent for it). They differ only in buffer lifetime — the sequential form holds ONE pass's masked-forward residuals at a time — and, at bf16 compute, in backward rounding: measured on the TMS fixture, every loss scalar is BIT-IDENTICAL between the two and the decomposition agrees to ~1e-7 relative, while gradient-NORM diagnostics differ by ~4e-4; forcing fp32 compute makes the decomposition bit-identical and drops the norm gap to ~1e-7, and re-summing the same per-pass gradients in a different order accounts for only ~3e-8, so the residual is bf16 rounding rather than reassociation or a different objective. Pinned by `core/tests/test_dual_objective.py`. |
| T2 | Two independent streams, each an ordinary `sample_batch(step)` seam. The TARGET stream's global batch is `pd.batch_size` and its geometry is the run's `positions` — everything core sizes off those (persistent sources, S16/D1 shapes) is target-pass state. The non-target stream's global batch is `nontarget.batch_size`, its sequence length the broad dataset's own. **AMENDED 2026-08-19** (with T5/T7, pending Oli sign-off): bundle sizing is PER-PASS, not per-run — a non-target OUTPUT term's persistent bundle sizes off ITS pass's geometry (`nontarget.batch_size` x the broad seq len, threaded to state init as `nontarget_positions`), never `pd.batch_size`. |
...
| T4 | Every non-target forward of a delta-pinned term runs with every weight-delta mask pinned to `1.0`. Component masks compose per S1 from the NON-TARGET batch's own CI envelope; constant-source entries (which carry no delta path in the plain objective) carry a pinned-on delta here. ONE enumerated exception: an `UnmaskedNoDeltaReconLoss` term runs every component mask at `1.0` and every weight-delta mask at `0.0` — the paper's unmasked reconstruction term for CSS-only decompositions (all `m_i = 1`, `m_Δ = 0`; Method details), which makes the FULL component sum alone reconstruct so that dead components that never activate cannot interfere with the reconstruction. The polarity rides its own strategy type (`UnmaskedNoDeltaSources`), never a flag on the pinned arms. **Amended 2026-08-06**: was "every non-target forward"; the unmasked-no-delta arm is the one delta-off exception. **AMENDED 2026-08-20** (pending Oli sign-off): the rule extends to the EVAL surface — a targeted run's non-target-stream fresh-PGD probe (`PGDReconLoss`, both CI roles) composes DELTA-PINNED (`delta_pinned` through `recon_eval.fresh_pgd_recon_*` / `lm.eval.make_fresh_pgd_step`, bound in `make_fresh_pgd_operation`). Before this the probe handed the adversary the sources' delta channel, so its dominant off-target move was `m_Δ -> 0` — an attack on the escape valve every training forward pins shut, costing at least the CSS-only floor (measured ~0.113 KL on addsub-L18) and swamping the component-only worst case the training can actually defend; every adversary arm (E2a/E2b/E2c, the 16-line from-scratch runs) read a flat ~0.25-0.27 for exactly this reason. Plain runs and the target stream keep the delta channel attackable, unchanged. **AMENDED 2026-08-28** (pending Oli sign-off): the eval-surface rule covers the whole non-target recon eval family, not just the fresh-PGD probe — a targeted run's non-target-stream `CEandKLLosses` variants and the standalone masked-KL arms compose delta-pinned (`delta_pinned` through `lm.eval.make_ce_kl_step` / `make_masked_kl_step`, bound via `scalar_eval_operations.nontarget_delta_pinned` in `make_ce_kl_operation` / `make_masked_kl_operation`, both CI roles): `ci_masked`/`random_masked`/`rounded_masked` pin every weight-delta mask to 1.0, `stoch_masked` pins its delta to 1.0 instead of drawing `U[0,1]` (the T4 training composition), `zero_masked` pins too (a constant-source-0 arm per this rule, so it reads the delta-only reconstruction off-target), and `kl_unmasked` keeps the enumerated delta-off exception. Before this, every non-target `ce_kl/*` forward ran delta-off, measuring a component-only quantity no training forward defends — the same escape-valve mismatch this rule's 2026-08-20 amendment fixed for the probe, which is why those keys sat persistently high while the pinned probe did not. Plain runs and the target stream are unchanged. |
| T5 | The non-target surface is closed: delta-pinned stochastic/constant recon (KL against the broad stream's frozen clean output, scored full-length), the unmasked-no-delta term (T4's one delta-off exception), and imp-min — nothing else. Adversarial and mixed sources are unrepresentable in `NontargetReconLossMetricConfig`; the S35 hidden-acts rider is refused on non-target terms; and the narrowing is carried IN THE TYPE — `NontargetPass.recon` is `ReconLossTerm[StochasticSources | ConstantSources | UnmaskedNoDeltaSources]`, so the non-target grid's dispatch is exhaustive over exactly the three enumerated arms, with one terse factory assert as the library boundary for objectives built past the type. **Amended 2026-08-06**: admitted `UnmaskedNoDeltaReconLoss` (T4's exception) into the closed set. **AMENDED 2026-08-18**: the non-target stream may carry a HIDDEN pass (`nontarget.hidden`, T12) — importance-minimality on the hidden head plus delta-pinned recon scored at the hidden pass's `points`. The recon VOCABULARY is unchanged (the same closed non-target set, still no adversarial or mixed sources — T7 is untouched), and the type-level narrowing still holds: `HiddenPass[StochasticSources | ConstantSources | UnmaskedNoDeltaSources]`. What changes is the refusal of internal-activation matching off-target. The original argument for that refusal stands on its own terms — with the delta pinned on, matching internal activations constrains the behavior tPD declines to decompose — and it is recorded here rather than deleted, because a hidden pass off-target is an EXPERIMENTAL choice this amendment enables, not a claim that the argument was wrong. It is optional and absent by default; `nontarget.hidden` requires `pd.hidden`, whose `points` it measures at. **AMENDED 2026-08-19** (pending Oli sign-off): the non-target OUTPUT pass additionally admits the two persistent-source-carrying recon types (`PersistentPGDReconLoss`, `MergedStochasticSubsetPPGDReconLoss`) — `NontargetConfig.recon` is `NontargetOutputReconLossMetricConfig`; `NontargetPass.recon` widens to `… | PersistentSources | MixedPersistentStochasticSources`. Their masks compose from the bundle's COMPONENT channels with every weight-delta mask pinned to 1.0 (T4; `persistent_delta_pinned_masks` / the S34 merge's delta-pinned twin), so the probed worst case is a partial ablation of components on data they should be inactive on — the delta path the refusal argument protects stays pinned. State keys carry a `nontarget/` prefix (the adversary namespace is global). Fresh-PGD and the S35 rider remain unrepresentable off-target; the non-target HIDDEN vocabulary is unchanged (worst-case hidden-acts robustness off-target is out of scope by decision). Motivation: eval `nontarget_data/loss/PGDReconLoss` sat ~60x the target stream's on addsub-L18-14-4x (0.259 vs 0.0044 @ step 16k) with no training pressure against it. **AMENDED 2026-08-20** (pending Oli sign-off): the non-target HIDDEN pass additionally admits the MERGED type only (`NontargetHiddenReconLossMetricConfig` = the closed set + `MergedStochasticSubsetPPGDReconLoss`; `HiddenPass[NontargetHiddenSources]` widens by `MixedPersistentStochasticSources`) — the one adversarial arm whose masked forward REPLACES a stochastic term's (S34), so the admission costs the broad stream zero extra forwards. The standalone `PersistentPGDReconLoss` stays refused there (it would be an extra forward per step; dedicated compute for off-target hidden robustness remains out of scope). State keys carry a `nontarget_hidden/` prefix; bundles size off the broad stream's geometry per T2's per-pass rule; delta pinned per T4 as on every non-target forward. |
...
| T7 | Adversaries belong to the pass that AUTHORS them, and each runs against its own stream, CI head, and objective: warmup ascents (S24) score on the owning pass's stream (delta-pinned per T4 off-target), the final ascent rides the shared backward (S14′), and sizing follows T2's per-pass rule. Fresh-PGD adversaries exist in TARGET-stream passes only. **AMENDED 2026-08-19** (pending Oli sign-off; formerly "target pass only" outright): the non-target OUTPUT pass may carry persistent bundles per the T5 amendment. **AMENDED 2026-08-20**: the non-target HIDDEN pass may carry a merged term's bundle per T5's 2026-08-20 amendment (ascended against its own stream and the hidden head, delta pinned; with `n_warmup_steps: 0` its only update is the S14' final ascent). Each bundle still feeds exactly one term (S23 — no cross-stream source sharing). |
...
| T10 | The TARGET pass treats the delta exactly as plain VPD does: stochastic entries draw a `U[0,1]` delta source, adversarial entries carry their source's delta channel (S1). **OPEN** — the algorithm's author-definition may instead pin the delta OFF on-target ("components alone reconstruct the target behavior"); every implementation to date, and every recorded run, uses the stochastic form. Resolving this changes only the target pass's mask derivation. |
...
| T12 | The HIDDEN pass (S36's second role, given an objective). A targeted run may carry a second reconstruction objective per stream, scored at named internal activations instead of the model output: `pd.hidden` authors the target stream's (`points`, `impmin_coeff`, `recon`), `nontarget.hidden` the broad stream's. Its masks come from the CI fn's hidden head; its comparison is `HiddenActsOnlyReconstruction(points)` — the point mean of per-point relative squared error and NOTHING ELSE, so `recon_loss_fn` is never called on a hidden forward and the e2e KL never enters that pass's graph. This is why the hidden role is a PASS and not S35's per-term rider: the rider adds `coeff · hidden` ON TOP of a term's e2e loss, so its CI stays shaped mostly by the output objective, whereas the hidden pass asks its own question with nothing diluting it. `points` lives on the pass, not the term — the pass IS "reconstruct these activations" — and is SHARED with the non-target hidden pass, the way T6 shares one imp-min config: WHAT is reconstructed is a property of the experiment, not of a stream; only coefficients differ per pass. The imp-min penalty config is likewise the target pass's (T6 extended to the role); only `impmin_coeff` is the hidden pass's own. A hidden pass may carry adversaries per T7 (target stream: the full vocabulary; non-target stream: the merged term only, per the 2026-08-20 T5 amendment) and ascends them against its OWN head and objective — never the output pass's. Loss identities must be unique across the target and hidden passes (they share a metric namespace); refused at parse. Metric namespaces compose the two axes: `nontarget_data/` for the stream, `hidden_ci/` for the role, so the target-output pass keeps every key it has always logged. RNG offsets (T9) run target-output, non-target, then the hidden passes, so adding a hidden pass does not move an existing run's draws. Absent by default and byte-inert when absent: the step is the pre-T12 two-pass step. **Added 2026-08-18.** |
