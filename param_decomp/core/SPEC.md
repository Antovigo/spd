...
| T14 | **CI-ordering ("magenta") penalty** (`pd.hidden.ci_anomaly_penalty: {coeff, form}`, targeted-only, requires the hidden pass, T12). The hidden points sit upstream of the model output, so a subcomponent important for output reconstruction should be important for some hidden activation too: the expected ordering is `hidden_ci >= output_ci` per CI value, and `output_ci > hidden_ci` is the anomaly. The term is `Σ_s Σ_c mean_{b,t} psi(relu(upper_output − sg(upper_hidden)))` over the TARGET stream's two heads, `psi` the identity (`form: linear`) or the square (`form: squared`) — the same reduction as imp-min's activity term (S8), so `coeff` shares `impmin_coeff`'s scale. Both sides are the `upper` squashing (S6), so the pushed head keeps a gradient above 1; the hidden side is stop-gradient, so the penalty only ever LOWERS output CI and never raises hidden CI; it is exactly zero in value and gradient wherever the ordering already holds. Scored ONCE per step, inside the target-OUTPUT pass (whose head it moves), off that pass's own pre-update CI bundle; `coeff · term` enters the differentiated and the reported totals; logged unweighted as `loss/CIAnomalyPenalty`, its schedule (if any) under `schedules/coeff/CIAnomalyPenalty`. The non-target stream is NOT penalized (measurement only, below). Byte-inert when absent. The `CIAnomaly` eval (fast tier; `experiments.lm`) measures the same statistic per STREAM on any dual run without training it: `ci_anomaly/linear`, `ci_anomaly/squared` (the two forms at the training reduction), `ci_anomaly/linear/<site>`, and `ci_anomaly/violating_fraction` (the share of CI values with `output > hidden`), bare for the target pool and under `nontarget_data/` for the broad corpus; it reads both heads, so it carries no role segment. **Added 2026-09-05.** |
