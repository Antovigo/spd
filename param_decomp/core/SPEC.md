...
| T11 | CI-scaled weight decay on subcomponent vectors (`TargetedPDConfig.ci_scaled_weight_decay`, optional — absent means no decay, and absence is byte-inert: the step is the pre-T11 step). AFTER each optimizer step, every subcomponent's V column and U row are scaled by `keep_c = 1 − lr(step)·wd·(1 − max_c)`: `wd` the authored coefficient, `lr(step)` the components optimizer's CURRENT scheduled LR (AdamW's decoupled-decay convention), `max_c` that subcomponent's max CI over the SELECTED passes' batches — every leading axis, batch and positions alike; a component important on any selected pass is not dead. The SELECTION is three optional fields, each defaulting to the original rule, so a config that names none of them runs the pre-amendment step byte-for-byte (amended 2026-09-14): `ci_scaled_weight_decay_stream` (`all` | `target` | `nontarget`) drops a stream's passes from the reduction — under `target`, a component load-bearing only on the broad stream IS decayed; `ci_scaled_weight_decay_role` (`all` | `output` | `hidden`) drops a CI READOUT HEAD's passes — under `output`, a component that only ever matters to the hidden-activations objective (T12) IS decayed, and under `hidden` the converse; `ci_scaled_weight_decay_quantile` (`(0, 1]`, default `1.0` = the max) replaces each selected pass's per-component batch max with that quantile over its pooled leading axes, so aliveness on a handful of points no longer buys full escape. The per-pass statistic is combined ACROSS passes by max regardless of quantile. A selection matching no pass the objective built is a construction-time error, not a silently full-rate decay. The CI read is the step's own pre-update forward's `lower` (no clamp needed: `lower ≡ clip(upper, 0, 1)` pointwise per S6, so the two squashings agree on the statistic). This is an update rule, not a loss term: nothing differentiates through it, and the CI fn, imp-min, and the adversaries never see it. Targeted-only by design: in plain PD faithfulness penalizes `‖W − VU‖`, so shrinking a dead component's V/U GROWS the residual and the decay would fight faithfulness head-on; in tPD the delta is the free escape valve (T3), so the never-important components' stale vectors — which imp-min cannot shrink (it only pushes their CI down) — get dragged to zero. Ref: the tPD paper's "weight decay of 0.1 on subcomponent vectors, where the weight decay is scaled by 1 − max_batch(CI)". |
| T12 | The HIDDEN pass (S36's second role, given an objective). A targeted run may carry a second reconstruction objective per stream, scored at named internal activations instead of the model output: `pd.hidden` authors the target stream's (`points`, `impmin_coeff`, `recon`), `nontarget.hidden` the broad stream's. Its masks come from the CI fn's hidden head; its comparison is `HiddenActsOnlyReconstruction(points, normalization)` — the point MEAN of per-point relative squared error and NOTHING ELSE (each point at weight 1/P, so adding points dilutes the others unless the term coefficients are rescaled by P/P_old), so `recon_loss_fn` is never called on a hidden forward and the e2e KL never enters that pass's graph. This is why the hidden role is a PASS and not S35's per-term rider: the rider adds `coeff · hidden` ON TOP of a term's e2e loss, so its CI stays shaped mostly by the output objective, whereas the hidden pass asks its own question with nothing diluting it. `points` lives on the pass, not the term — the pass IS "reconstruct these activations" — and is SHARED with the non-target hidden pass, the way T6 shares one imp-min config: WHAT is reconstructed is a property of the experiment, not of a stream; only coefficients differ per pass. The imp-min penalty config is likewise the target pass's (T6 extended to the role); only `impmin_coeff` is the hidden pass's own. A hidden pass may carry adversaries per T7 (target stream: the full vocabulary; non-target stream: the merged term only, per the 2026-08-20 T5 amendment) and ascends them against its OWN head and objective — never the output pass's. Loss identities must be unique across the target and hidden passes (they share a metric namespace); refused at parse. Metric namespaces compose the two axes: `nontarget_data/` for the stream, `hidden_ci/` for the role, so the target-output pass keeps every key it has always logged. `pd.hidden.normalization` (S35's `batch` | `per_position`, amended 2026-09-11) is the pass's and is shared with the non-target hidden pass exactly like `points`. **Amended 2026-09-11, e2e rider:** `pd.hidden.output_coeff` / `nontarget.hidden.output_coeff` (optional, per pass, `LossCoeff`) adds `output_coeff · kl_per_position` to every hidden term's comparison on the SAME masked forward (`HiddenActsAndOutputReconstruction`: `mean_points + output_coeff · KL`, the bare KL logged as `<term>/e2e`) — the mirror image of S35's rider, asking the hidden head to also keep what the output needs. Absent, the pass is exactly the pure hidden pass above (`recon_loss_fn` never called). A hidden pass's adversaries ascend its complete objective, rider included (the `e2e` substitution stays an output-pass notion). RNG offsets (T9) run target-output, non-target, then the hidden passes, so adding a hidden pass does not move an existing run's draws. Absent by default and byte-inert when absent: the step is the pre-T12 two-pass step. **Added 2026-08-18.** |
| T13 | **Neuron-aligned targeted init** (`decomposition.sites.initialization: neuron_aligned_targeted`, targeted-only; the plain LM schema refuses it at parse; a `ComponentInitializer` like the data-free `neuron_aligned`). EVERY decomposed site's subcomponent `i` STARTS as one architectural coordinate of its matrix — the `i`-th of the site's kind's ranking on the target prompt pool: with `W [d_out, d_in]` the frozen weight, `S` the ranking and `E [C, n]` the selection matrix `E[i, S_i] = 1`, a kind whose units sit on `d_out` (q/k/v; gate/up, the plain anatomy's fc) takes `V = W[S_{:C}, :]ᵀ, U = E` and one whose units sit on `d_in` (o, down — the anatomy's row kinds) takes `V = Eᵀ, U = W[:, S_{:C}]ᵀ`, so `(V@U)ᵀ` is `W` restricted to the selected rows / columns and the delta carries the other `n − C` coordinates exactly (T3's escape valve). Nothing is frozen and nothing is perturbed: one-hot is the gauge in which each subcomponent IS one coordinate; per subcomponent `‖v_i‖·‖u_i‖` equals the coordinate's weight norm, and both factors have dense nonzero gradients at step 0. C is free per site (`C ≤ n`, refused at resolve); each site takes the PREFIX of ONE per-(block, kind) ranking, so a block's gate/up neurons nest inside its down's when `C_down ≥ C_gate`. The RANKINGS are statistics of the target model and the target prompt pool alone, per block and ranking family: MLP neurons by write energy `E[h_i²]·‖W_down[:, i]‖²` (`h` the block's post-nonlinearity hidden, the `mlp_hidden` tap; one ranking for gate/up/down); `q` and `k` channels by their own projection-output energy `E[q_j²]`, `E[k_j²]` (their `.out` taps, pre-RoPE — attention mixes across positions, never across channels, so `q_j` and `k_j` interact only inside their head's dot product and rank independently); `o` channels by the write energy of the attention-core output `E[z_j²]·‖W_o[:, j]‖²` (the `attention_output` tap); `v` channels by that same write energy summed over the kv group's query heads (v channel `(g, d)` IS attention-output channel `(h, d)` for every `h` in group `g`). Uncentred, the expectation over EVERY pool prompt exactly once (an exhaustive sweep, never the trainer's with-replacement sampler) and over every position, descending with index tie-break — and depending on no decomposition setting, so a HARVESTED ARTIFACT (`experiments.lm.harvest_neuron_ranks` → `neuron_ranks.npz` with `rank_<kind>_<b>` / `score_<kind>_<b>` + `meta.json`, statistic `unit_energy`; referenced by `decomposition.sites.neuron_ranks`, required iff the init is selected) whose provenance — target identity, the pool's token fingerprint, every decomposed block ranked — is asserted at load; any drift refuses, as does an artifact of another statistic. The trainer never sweeps; a requeue / fine-tune reads the same file and restores over the aligned reference. Byte-inert when not selected. **Added 2026-09-03; every site kind aligned and the `coupled`/`zero_u` arms retired 2026-09-04.** |
