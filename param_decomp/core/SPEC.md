...
| T13 | **Neuron-aligned init** (`pd.weight_init: neuron_aligned_targeted`, targeted-only; `PDConfig` refuses it at parse). Every decomposed MLP site's subcomponent `i` STARTS as one neuron of its block: with `W [d_out, d_in]` the site's frozen weight, `S` the block's neuron ranking and `E [C, n]` the selection matrix `E[i, S_i] = 1`, a hidden WRITER (gate/up; the plain anatomy's fc — neuron axis `d_out`) takes `V = W[S_{:C}, :]ᵀ, U = E` and the READER (down — neuron axis `d_in`) takes `V = Eᵀ, U = W[:, S_{:C}]ᵀ`, so `(V@U)ᵀ` is `W` restricted to the selected rows / columns and the delta carries the other `n − C` neurons exactly (T3's escape valve, as under `coupled`). Nothing is frozen and nothing is perturbed: one-hot is the gauge in which each subcomponent IS one neuron; per subcomponent `‖v_i‖·‖u_i‖` equals the neuron's weight norm, and both factors have dense nonzero gradients at step 0. C is free per site; each site takes the PREFIX of ONE per-block ranking, so a block's writers' neurons nest inside its reader's when `C_down ≥ C_gate`. Every non-MLP site takes the `zero_u` values, bit-identical to a `zero_u` run at the same seed (the per-site keys are split as `coupled` splits them and an aligned site does not consume its key). The RANKING is a statistic of the target model and the target prompt pool alone — `score_i = E_tokens[h_i²] · ‖W_down[:, i]‖²` (write energy), `h` the block's post-nonlinearity hidden activation (`mlp_hidden` tap), uncentred, the expectation over EVERY pool prompt exactly once (an exhaustive sweep, never the trainer's with-replacement sampler) and over every position or every position but BOS (the artifact's `bos` policy), descending with index tie-break — and depends on no decomposition setting, so it is a HARVESTED ARTIFACT (`experiments.lm.harvest_neuron_ranks` → `neuron_ranks.npz` + `meta.json`, referenced by `pd.neuron_ranks`, required iff the init is selected) whose provenance — target identity, the pool's token fingerprint, every decomposed MLP block ranked — is asserted at load; any drift refuses. The trainer never sweeps; a requeue / fine-tune reads the same file and restores over the aligned reference. Byte-inert when not selected. **Added 2026-09-03.** **Amended 2026-09-04 — `neuron_aligned_wrap`:** the same ranking and artifact, but EVERY neuron is assigned, rank `j` to subcomponent `j mod C`, so with `E [C, n]` the (multi-hot) assignment matrix the formulas above hold verbatim — writer `V = (E@W)ᵀ, U = E`; reader `V = Eᵀ, U = E@Wᵀ` — and a subcomponent starts as the SUM of its `⌈n/C⌉` neurons: it reads the sum of their input weights and writes the same value to all of them (writer), reads all of them and writes the sum of their output weights (reader). `(V@U)ᵀ` is then `EᵀE·W` (writer) / `W·EᵀE` (reader) — each neuron's row/column replaced by its slot's group sum — and the delta carries `W − (V@U)ᵀ`, which is NOT small: the wrapped start is an every-neuron-touched start at ~⌈n/C⌉× a neuron's norm, not an exact one. The exact arm is the special case `E` one-hot on the top-C. Slot `i` must hold the SAME neuron group in a block's gate, up and down (the group's `gate·up` product is what `down` reads), and `j mod C` only agrees across the block's sites when they share ONE C — so `wrap` REFUSES a block whose MLP sites differ in C (C is a pinned structural property: checkpoints, placement, the CI heads — never rewritten at init); author them equal, at the MEAN of the sizes being compared against so the subcomponent total is unchanged. |
