...
class DataConfig:
    """The LM pre-tokenized parquet data source: a directory of shards. `None` on
    `BuiltRun` for a non-LM (toy) run, whose synthetic data is generated by its lab
    `sample_batch`. The global batch is NOT here — it is `pd.batch_size`, uniformly for
    every target. The loader's `seq_len` is an explicit parameter (`ShardServer`), and
    the dir is not required to exist until load time — nothing stats it at config build."""
