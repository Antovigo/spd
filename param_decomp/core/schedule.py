...
quantity (main LRs, PPGD source LR, imp-min `p`/`gamma`, merged-loss
`adv_fraction`) is configured by `ScheduleConfig` and evaluated in-step by the jnp twin
`losses.scheduled_value_traced` (jax lives there so this module — imported by the config
schema — stays jax-free); `test_schedule.py` pins the pair pointwise."""
