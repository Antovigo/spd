...
def persistent_delta_pinned_masks(
    ...
    for site, ci in ci_lower.items():
        masks[site] = ci + (1.0 - ci) * sources[site].components.astype(ci.dtype)
        delta_masks[site] = jnp.ones(ci.shape[:-1], ci.dtype)
    return masks, delta_masks
...
def mixed_persistent_stochastic_delta_pinned_masks(
    ...
    fresh_uniform_components = {
        site: uniform_like(random.fold_in(uniform_key, site_idx), ci, dtype=jnp.float32)
        for site_idx, (site, ci) in enumerate(ci_lower.items())
    }
    adv_masks, _ = persistent_delta_pinned_masks(ci_lower, persistent_sources)
    masks: dict[str, Array] = {}
    delta_masks: dict[str, Array] = {}
    for site, ci in ci_lower.items():
        stoch = ci + (1.0 - ci) * fresh_uniform_components[site].astype(ci.dtype)
        selector = adversarial[..., None].astype(ci.dtype)
        # Arithmetic `where` for the same reason the plain merge uses one: a replicated
        # selector against axis-typed arms is fine under broadcast rules, but not under
        # `select_n`.
        masks[site] = selector * adv_masks[site] + (1 - selector) * stoch
        delta_masks[site] = jnp.ones(ci.shape[:-1], ci.dtype)
    routes = (
        None
