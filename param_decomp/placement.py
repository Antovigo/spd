...
The ab-initio sharding design. Three vocabularies:
...
never re-decide (the decision is made at build time).
...
class PlacedRule:
    ...
    def transition_bytes(
        ...
        and axis locality. Good enough to rank
...
class ParamsPlacement:
    ...
    declared intent for the staged migration (the
...
# — this constant is where the trainer says so.
...
def _assign_groups(
    ...
    """THE tiles-or-fallback decision, made once, at construction: a shape group persists on `persist` when its stack length
...
def _preset(
    ...
    `owner` (stack ÷replicate, d ÷fsdp — the muon-motivated owner-partitioned layout,
...
def component_stacks_shardings(
    ...
    owner-partitioned HSDP layout). Boundary-validated by
