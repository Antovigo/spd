...
def adapter_from_config(method_config: ParamDecompHarvestConfig, data_root: Path) -> PDAdapter:
    assert is_jax_run(data_root, method_config.wandb_path), (
        f"{method_config.wandb_path}: not a loadable PD run (missing launch_config.yaml or orbax ckpts/)."
    )
    return PDAdapter(method_config.wandb_path, data_root)


def adapter_from_id(decomposition_id: str, data_root: Path) -> PDAdapter:
    ...
    repo = HarvestRepo.open_most_recent(decomposition_id, data_root)
    assert repo is not None, (
        f"No harvest data found for {decomposition_id!r}. "
        f"Run a harvest first (it stores the method config this adapter reads)."
    )
    ...
    return adapter_from_config(method_config, data_root)
