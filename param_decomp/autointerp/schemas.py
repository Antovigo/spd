...
def get_autointerp_dir(data_root: Path, decomposition_id: str) -> Path:
    """Top-level autointerp directory for a decomposition."""
    return data_root / "runs" / decomposition_id / "autointerp"


def get_autointerp_subrun_dir(
    data_root: Path, decomposition_id: str, autointerp_run_id: str
) -> Path:
    """Directory for a specific autointerp run (timestamped subdirectory)."""
    return get_autointerp_dir(data_root, decomposition_id) / autointerp_run_id
