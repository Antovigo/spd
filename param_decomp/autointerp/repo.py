...
Owns `<data_root>/runs/<run_id>/autointerp/` and provides read access to
...
class InterpRepo:
    ...
    @classmethod
    def _find_latest_done_subrun_dir(cls, run_id: str, data_root: Path) -> Path | None:
        autointerp_dir = get_autointerp_dir(data_root, run_id)
    ...
    @classmethod
    def open(cls, run_id: str, data_root: Path) -> "InterpRepo | None":
        """Open autointerp data for a run. Returns None if no completed autointerp data exists."""
        subrun_dir = cls._find_latest_done_subrun_dir(run_id, data_root)
    ...
    @classmethod
    def open_subrun(cls, run_id: str, subrun_id: str, data_root: Path) -> "InterpRepo":
        """Open a specific autointerp subrun by ID."""
        subrun_dir = get_autointerp_dir(data_root, run_id) / subrun_id
        db_path = subrun_dir / "interp.db"
