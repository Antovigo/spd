...
class HiddenActsReconAux(BaseConfig):
    """Auxiliary hidden-activation reconstruction riding on a recon loss (SPEC S31).

    Adds `coeff * MSE(masked_site_output, frozen x@W)` — per decomposed site's OUTPUT
    activation, a site-local objective distinct from the KL-on-final-logits recon — to the
    training loss, sharing the host recon term's masked forward (the per-site outputs are
    collected from that forward; the frozen targets from the clean forward already run each
    step, so NO extra forward). The MSE covers exactly the sites ablated in each forward AND
    the positions routed True (an activation directly after a replaced matrix), and is logged
    as `loss/<host>/hidden_acts` whenever configured — `coeff: 0.0` measures it without
    affecting training."""
