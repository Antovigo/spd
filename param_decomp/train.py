...
the recon loss TERMS (`recon.py`; each term = plan × mask-source strategy) → one fused backward over (components, ci_fn, all persistent sources) →
optimizer updates → each persistent term's final ascent from the same graph,
unscaled by ITS coeff. All trainable state is fp32 masters; forwards run in bf16 via explicit casts.

Schedules (imp-min p anneal, source-LR warmup) are computed inside the step from
`state.step`, so the jit signature is stable across the whole run.
Per-term RNG: term i draws from `fold_in(step_key, 1 + i)` in config-list order — this reproduces the pre-unification production key derivation exactly.
"""
...
def make_train_step(
    ...
    *,
    ...
    def ci_batch_sharded(ci: CI) -> CI:
        ...
        pins `x@V` to, so the downstream mask multiply `xV * mask` needs no
    ...
    def entry_loss_for_sources(
        ...
        ascent objective (shared by fresh and persistent ascents). `prepared` is
    ...
    def step(
        ...
        # all-sites forward (torch warmup parity, NOT the term's loss plan),
        # params + CI detached. The warmed sources then enter the main backward as leaves;
        # the LR schedule lives in `PersistentAdversary`. ──
        ...
        # ascents and the main loss forward; sign-ascend `n_steps`, then
        ...
        def draw_recon_loss(
            ...
            masked forward, compare against the clean output."""
        ...
        def term_draws(
            ...
            reproduces the pre-unification production trace exactly; fresh-PGD
            entries reuse the ascent phase's fixed routes."""
            term_key = random.fold_in(key, 1 + term_idx)
        ...
        # ── each adversary's final ascent from the fused graph: the
        # backward saw coeff·L_term, so it ascends on L_term itself (unscaled by its coeff
        # inside `final_ascend`, exact since one source bundle feeds one term). ──
        new_adversaries = {
