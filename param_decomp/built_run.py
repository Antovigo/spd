...
class TargetSites(Protocol):
    ...
    sites. LM target configs live in the composition layer; non-LM targets (the toys)
    supply their own config dataclass — the core never names them."""
...
class EvalConfig:
    ...
    hidden-acts recon) runs every `slow_every` steps, reusing the same eval batches. `rounding_threshold` binarises CI for the CE/KL `rounded_masked`
...
class BuiltRun:
    ...
    LM target config or a toy target config (satisfying `TargetSites`)."""
