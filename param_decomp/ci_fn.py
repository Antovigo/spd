...
The SAME logits are squashed two ways in ONE place (`CI.from_logits`):
...
class Chunk:
    ...
    Authored by the composition root (from `blocks_per_chunk` + topology); core treats both keyspaces as
...
class ChunkTransformer(eqx.Module):
    ...
    split per site (matching `x@V` / the mask in `site_out`). Under pure HSDP the C
