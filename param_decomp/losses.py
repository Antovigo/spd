...
def scheduled_value_traced(step_f32: Array, total_steps: int, config: ScheduleConfig) -> Array:
    ...
    `final_val_frac ×` at `step = total_steps - 1`. Plain
...
def kl_per_position(
    ...
    `*leading` is every axis but the final logit axis."""
...
def faithfulness_loss(weight_deltas: dict[str, Float[Array, "_ _"]]) -> Float[Array, ""]:
    """`Σ_s ‖Δ_s‖² / Σ_s numel` over fp32 deltas. Each `Δ_s` is `(d_out, d_in)`;
...
def _imp_min_terms(
    ...
    """`(lp, freq)` for any per-value penalty `psi`, with per-site grouping:
...
def annealed_imp_min_param(
    ...
    smooth-L0). Pure in the step, so the train step hoists it out of the
    loss `grad`."""
    match cfg:
