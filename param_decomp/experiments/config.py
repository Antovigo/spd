...
class ExperimentConfig(BaseConfig):
    ...
    def _strip_removed_run_identity_fields(cls, data: object) -> object:
        ...
        # `out_dir` (vestigial; the run dir is `<data_root>/runs/<run_id>`). Both
...
def run_instance(cfg: ExperimentConfig, run_id: str, data_root: Path) -> RunInstance:
    """The resolved run identity + logging lineage. `run_id` is minted by the launcher (a
    toy mints its own); the run dir is `<data_root>/runs/<run_id>`."""
