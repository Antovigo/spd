...
class ChunkwiseTransformerCiConfig(BaseConfig):
    ...
    dual: bool = Field(
        default=False,
        description=(
            "Give the CI fn a SECOND readout head on this same trunk (SPEC S37): the input "
...
class GlobalMlpCiConfig(BaseConfig):
    ...
    dual: bool = Field(
        default=False,
        description=(
            "Give the CI fn a SECOND readout head on this same trunk (SPEC S37): the "
...
def _assert_hidden_pass_has_a_head(cfg: LMTargetedExperimentConfig) -> None:
    ...
    assert dual == hidden, (
        "decomposition.ci.dual and pd.hidden must be set together (SPEC S37/T12): "
