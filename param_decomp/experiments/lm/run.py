...
def assert_finetune_structural_compat(
    built: BuiltRun, prov: ResumeProvenance, data_root: Path
) -> None:
    ...
    parent = load_run_dir_config(prov.parent_run_dir, data_root)
...
def train(
    ...
    seq_len = read_dataset_meta(data.dir).seq_len
...
def _make_lm_eval_fn(
    ...
    eval_server = ShardServer(
        eval_schedule, read_dataset_meta(data.dir).seq_len, jax.process_index(), n_proc
    )
...
def main(config: Path, run_id: str | None = None, data_root: Path = DEFAULT_DATA_ROOT) -> None:
    ...
    built, _raw_cfg = load_config(config, run_id, data_root)
