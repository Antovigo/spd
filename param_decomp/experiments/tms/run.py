...
SLURM / `param_decomp.core.run` / CUDA). It mints its own `p-<8hex>` run id (toys do not go
through the LM submit path).
"""
...
def build_tms_built_run(cfg: TMSExperimentConfig, run_id: str, data_root: Path) -> BuiltRun:
...
def main(
    ...
    built = build_tms_built_run(TMSExperimentConfig(**schema_raw), run_id, data_root)
