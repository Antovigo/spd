...
def test_targeted_two_pass_step_trains():
    ...
    assert "loss/nontarget_data/StochasticReconLoss" in metrics
    assert "loss/nontarget_data/total" in metrics
...
def test_targeted_step_unmasked_no_delta_scores_components_only():
    ...
    reported = metrics["loss/nontarget_data/UnmaskedNoDeltaReconLoss"]
...
def test_targeted_engine_end_to_end(tmp_path: Path):
    ...
    assert "train/loss/nontarget_data/total" in last
