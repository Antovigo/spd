...
from param_decomp_config.experiment import (
    ExperimentConfig as SchemaExperimentConfig,
)
from param_decomp_config.experiment import (
    ResumeProvenance,
    WandbConfig,
...
class TargetSites(Protocol):
    ...
    sites. LM target configs live in this module; non-LM targets (the toys) live in the
    lab and supply their own config dataclass — the core never names them."""
...
"""The LM target configs the core builds. Non-LM targets (the toys) live in the lab and
satisfy the `TargetSites` protocol — the core's `ExperimentConfig.target` is typed by that
protocol, never by a closed union, so it accepts a lab target config too."""
...
class ExperimentConfig:
    ...
    LM target config (this module) or a lab toy target config (satisfying `TargetSites`)."""
...
def layerwise_mlp_ci_arch(cfg: "SchemaExperimentConfig[Any, Any]") -> MLPCIArch:
    """Extract the layerwise per-site MLP CI-fn arch (`fn_type=mlp`) from a schema config.
    Public so the lab toy providers (which build their own `ExperimentConfig`) reuse it."""
    ci = cfg.pd.ci_config
...
class SharedAlgorithmConfig:
    ...
    domain. Public so the lab toy providers reuse the exact same conversion."""
...
def convert_shared_algorithm_config(
    cfg: "SchemaExperimentConfig[Any, Any]",
...
def run_instance(cfg: "SchemaExperimentConfig[Any, Any]") -> tuple[str, str, Path]:
    ...
    Public so the lab toy providers reuse it."""
...
def load_config(config_path: Path) -> tuple[ExperimentConfig, dict[str, Any]]:
    ...
    and stamped into the workspace copy by `pd-lm` at submit time, so resumes derive
