...
(it mirrors the torch plotting-metric reductions on numpy
...
The slow tier runs IN-LOOP on `eval.slow_every` next to the fast pass (`run.py`), reusing
the fast pass's eval batches and logging `slow_eval/*` on the live `_step` axis from a
...
count (`hidden_acts_eval.py`). Those ride the `masked_site_outputs` model seam and are
emitted as scalars under the torch log keys (`<ClassName>/<site>` + a combined
`<ClassName>`).
