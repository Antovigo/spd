...
from param_decomp.core.model import ForwardResult, MaterializedMasking, ResidualStart
...
def test_no_prefix_when_block_zero_is_decomposed():
    ...
    with pytest.raises(AssertionError, match="no frozen prefix"):
        model.prefix_residual(_tokens(_cfg))
...
def test_clean_forward_from_residual_start_matches_tokens():
    ...
    assert jnp.array_equal(model.clean_forward(start).output, model.clean_forward(tokens).output)
...
def test_prefix_boundary_capture_is_the_start_activation():
    cfg, model = _model((3, 4))
    tokens = _tokens(cfg)
    prefix = model.prefix_residual(tokens)
    captured = model.clean_forward(tokens, frozenset((f"resid.{model.split_layer}",))).captures
...
def test_tokens_still_capture_inside_the_prefix():
    ...
    captures = model.clean_forward(tokens, wanted).captures
    reference = unsplit.clean_forward(tokens, wanted).captures
...
def test_capture_below_the_prefix_is_refused_from_a_residual_start():
    ...
    start = ResidualStart(model.prefix_residual(_tokens(_cfg)))
...
def test_weight_deltas_index_the_suffix_stack():
    ...
    assert set(deltas) == set(model.site_names)
