...
def _meta(n: int, layers: list[int], tokens: np.ndarray) -> NeuronRanksMeta:
...
def test_mlp_blocks_of_partial_blocks():
    ...
    assert blocks[1].writers == (site_name(1, "gate"), site_name(1, "up"))
    assert blocks[1].reader is None
...
def test_moments_match_a_hand_computation_from_the_tap():
    ...
    for block in blocks:
        h = np.asarray(captures[mlp_hidden_tap_key(block)], dtype=np.float64)
        ...
        Wd = np.asarray(frozen_mlp_down_weight(model, block))
        expected = (h * h).mean(axis=(0, 1)) * np.sum(Wd**2, axis=0)
...
def test_artifact_round_trip_and_provenance_refusals(tmp_path: Path):
    ...
    out = tmp_path / "ranks"
    write_neuron_ranks(out, _meta(n, layers, tokens), rank, score)
    ...
    rank[1][0] = -1
    write_neuron_ranks(tmp_path / "bad", _meta(n, layers, tokens), rank, score)
...
def test_alignment_from_ranks_takes_prefixes_per_site_axis():
    ...
    ranks = NeuronRanks(_meta(n, [1, 3], _pool(1, 2, 8)), rank, score)
    ...
    assert np.array_equal(np.asarray(down.neurons), rank[1][:6])  # nested prefix
