"""The trainer's loss surface: the `LossSurface` record — the faithfulness +
...
source strategy. Everything here is static structure
...
class ReconForward:
    ...
    def has_delta(self) -> bool:
        ...
        matmul is skipped entirely (static, retrace-safe). Every
...
class ReconLossTerm:
    ...
    `kl_per_position`. `name` is the torch `instance_key` (`cfg.name` or
...
class FaithfulnessTerm:
    """Weight-space term: `Σ_s ‖Δ_s‖² / Σ_s numel`. Carries no plan — its
...
class ImportanceMinimalityTerm:
    """CI-space imp-min + entropy term. Carries the config so the step reads
...
@dataclass(frozen=True)
class LossSurface:
    """The VPD objective's fixed shape — `L = c·faith + c·imp + Σ c·recon`.
...
def live_groups(sites: tuple[str, ...], k: int) -> list[LiveSet]:
    """Sequential size-`k` groups in canonical site order (torch chunkwise)."""
...
def make_plan(
    ...
    """One `ReconForward` per live-set: those sites live, the rest frozen `x@W`,
    with `n_samples` routing draws from `routing` over the live-set's own sites
    and the shared `sources`. The live-set choice (`all_sites_live`/`each_site_live`/
    `live_groups`) and the routing/source choices are orthogonal."""
    return tuple(
        ReconForward(
            live_sites=live_set,
...
def persistent_configs(
    ...
    """`state_key -> config` for every persistent-source-carrying recon term (each key feeds exactly one term). Derived from the terms, not stored separately — the
...
def build_loss_terms(
    ...
    exactly one faithfulness + one importance-minimality term,
    ...
    index, so it is semantically load-bearing. Stochastic terms read
    ...
    for cfg in loss_metrics:
        ...
        match cfg:
            ...
            case PersistentPGDReconLossConfig():
                ...
                recon_terms.append(recon(cfg, plan))
            case _:
                # StochasticHiddenActsReconLoss lands here by design: it is offline-eval
                # only, not a JAX training loss.
