...
def _build_ci_fn(model: PlacedModel, n_embd: int, key: jax.Array, dual: bool = False) -> PlacedCIFn:
...
def _preactivations(
    ...
    return output_ci(evaluate_ci(ci_fn, captures, remat=False)).preactivations
...
def test_reductions_match_hand_rolled_per_component():
    ...
    preactivations = ci_preactivations(
        ci_fn,
        capture_clean(model.model, residual, ci_fn.fn.capture_keys),
        remat=False,
        role="output",
    )
...
def _tiny_position_ci():
    ...
    ci_fn = _build_ci_fn(placed, cfg.n_embd, jax.random.PRNGKey(2))
