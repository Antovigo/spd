...
def test_zero_blocks_forward_shape_and_finiteness():
    ...
    for remat in (False, True):
        ci = output_ci(ci_fn(_taps(), remat=remat, placement=None))
...
def test_zero_blocks_is_exactly_position_local():
    ...
    def moved(n_blocks: int) -> jax.Array:
        ...
        base = output_ci(ci_fn(taps, remat=False, placement=None)).preactivations[SITE]
        pert = output_ci(ci_fn(perturbed, remat=False, placement=None)).preactivations[SITE]
