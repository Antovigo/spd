...
def test_tpd_three_step_golden():
    ...
    # RE-RECORDED for the upstream #1001 merge (2026-09-04). #1001 deleted the `L_p`
    # importance-minimality penalty and gave its NAME to the smooth-L0 one, so this
    # fixture's `pnorm=1.0` became `gamma=1.0` — a different per-value penalty, hence
    # different numbers from the same step. These are upstream's own recorded goldens,
    # which the merged step reproduces to 12 significant digits under this branch's
    # non-target key spelling (`nontarget_data/loss/total`, not `loss/nontarget/total`):
    # the two-pass tPD step and upstream's agree exactly.
    expected_per_step = (
        {
            "total": 1.427467726171e-02,
            "loss/StochasticReconLoss": 2.224520547315e-03,
            "loss/PersistentPGDReconLoss": 2.428050385788e-03,
            "nontarget_data/loss/total": 9.804574772716e-03,
        },
        {
            "total": 1.091865170747e-02,
            "loss/StochasticReconLoss": 1.462063402869e-03,
            "loss/PersistentPGDReconLoss": 1.721132895909e-03,
            "nontarget_data/loss/total": 7.962030358613e-03,
        },
        {
            "total": 9.593502618372e-03,
            "loss/StochasticReconLoss": 1.872145221569e-03,
            "loss/PersistentPGDReconLoss": 3.473097924143e-03,
            "nontarget_data/loss/total": 5.056168884039e-03,
        },
    )
...
def test_targeted_factory_refuses_adversarial_nontarget_surface():
    ...
    _, model, _state, _ = _tiny_setup(_loss_metrics(), _stochastic_nontarget())
    nontarget = NontargetConfig(
        batch_size=32, impmin_coeff=6e-3, recon=[StochasticReconLossConfig(coeff=1.0)]
    )
