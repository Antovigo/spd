...
Two kinds of check:
  ...
  * **Structural.** `test_structure_*` pin structural invariants that aren't a single number:
    the stochastic recon runs ONE forward PER CHUNK, recon is KL not MSE,
  ...
  * **Chunk-plan static-live gate.** `test_chunk_plan_static_live_gate` drives
...
(only if the math or fixtures change): run the torch reference implementation's
`torch_reference.py` in a torch venv and copy the resulting `torch_reference.json`
back here. The
...
_PENDING_REGEN = pytest.mark.xfail(
    reason=(
        ...
        "contract (torch reference implementation)."
    ),
    strict=False,
)
...
def test_chunk_plan_static_live_gate() -> None:
    """The masked forward under a layer-SPLITTING chunk plan. The production
...
def test_structure_stoch_is_per_chunk() -> None:
    """One forward per (chunk, sample), normalized by `n_chunks · n_samples`
...
def test_structure_ppgd_has_delta_channel() -> None:
    """PPGD masks interpolate `ci + (1-ci)*src[:, :C]`; the trailing channel
