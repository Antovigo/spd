"""Bounds the bf16-input imp-min cast-point seam.
...
exact values torch reduced; `imp_min_bf16_reference.json` holds the frozen torch-reference
`lp`/`entropy`. The JAX side reconstructs the bits and runs the real
`importance_minimality_terms`. Regenerate the golden from the torch reference implementation only
...
well under the 1e-3 tolerance pinned below.
"""
