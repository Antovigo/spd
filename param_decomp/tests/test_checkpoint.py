...
def test_muon_roundtrip_and_exact_resume(tmp_path: Path):
    """The muon components opt state (optax-partitioned muon/adam
...
def test_stacked_muon_roundtrip_and_exact_resume(tmp_path: Path):
    """`impl: stacked`: the stacked-NS muon state is optax's `MuonState` pytree
...
def test_muon_cross_impl_checkpoint_roundtrip(tmp_path: Path, save_impl: str, restore_impl: str):
    """`impl: optax` and `impl: stacked` carry the SAME `MuonState` pytree, so a
...
def test_sharded_roundtrip_bit_equal(tmp_path: Path):
    """A sharded `TrainState` at the PRODUCTION per-rank shape (the failure-prone
    save path) must round-trip
