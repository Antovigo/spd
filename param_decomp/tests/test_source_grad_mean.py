...
exact spot of the historical 3-pool SUM bug; nothing previously pinned the AVG.
...
def _source_grad(sharded: bool) -> dict[str, jax.Array]:
    ...
    sources, with components/CI frozen — the leaf whose cross-device
...
def test_source_leaf_grad_is_global_mean_not_sum():
    ...
    for name in single:
        ...
        assert max_abs_err <= max_allowed, (
            ...
            f"not the global MEAN (an all-reduce(SUM) bug would give ~{n_dev}× the mean)"
        )
        ...
        assert abs(ratio - 1.0) < 1e-3, (
            ...
            f"~{n_dev} would mean the replicated source leaf got an all-reduce(SUM), not MEAN"
        )
