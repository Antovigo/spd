...
against the pre-restructure stacked implementation (stacked `DecompVU`,
...
`out::final_{V,U,src}`) are stale and need a golden regen against the torch reference
implementation (in a torch venv), out of scope for the API migration.
"""
...
def test_chunk_plan_static_live_set_matches():
    ...
    static live-set realization of the live/frozen site split: each plan entry holds a STATIC `live_sites`
...
@pytest.mark.xfail(
    reason="needs a torch-reference golden regen: CI numerics changed. The CI fn moved from the "
    ...
    "`out::final_{V,U,src}`) is stale. Regen depends on the torch reference "
    "implementation (a torch venv), out of scope for the API migration. The CI-INDEPENDENT "
