...
This script runs against the pre-restructure stacked implementation (the stacked
`DecompVU` with `(L, ., .)` arrays, `LayerRange`, `llama_decomposed_lm(cfg,
layer_range, C)`); it does NOT run against the current per-site code — the names it
imports were restructured away. Regenerate from a checkout of that revision, e.g.:
...
site inputs and a 2-step training trajectory to reassociation tolerance (rel ~1e-5).
